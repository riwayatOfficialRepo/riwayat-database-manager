...
exports.up = (pgm) => {
  ...
  pgm.sql(`
    CREATE UNIQUE INDEX IF NOT EXISTS promotions_name_display_ci
      ON promotions USING btree (lower(name_display::text) ASC NULLS LAST);
  `);
};

exports.down = (pgm) => {
  pgm.sql(`DROP INDEX IF EXISTS promotions_name_display_ci`);
  pgm.dropTable('promotions', { ifExists: true, cascade: true });
};
