exports.up = (pgm) => {
  ...
  pgm.sql(`
    CREATE OR REPLACE TRIGGER trg_update_feedbacks
...
exports.down = (pgm) => {
  pgm.sql(`DROP TRIGGER IF EXISTS trg_update_feedbacks ON feedbacks`);
  pgm.dropTable('feedbacks', { ifExists: true, cascade: true });
  pgm.sql(`DROP FUNCTION IF EXISTS update_feedbacks_updated_at()`);
};
